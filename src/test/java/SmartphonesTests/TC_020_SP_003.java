 ...
 * 4. Click on the "Sort By" button and select the "Lowest Price" option.
 * 5. Click on the "OppoF19", "Samsung Universe 9" and "iPhone X" Add to Cart buttons.
 * 6. Verify that the product is added in shopping cart.
 * 7. Click on the shopping cart icon.
 * 8. Verify that the products are added in shopping cart.
 * 9. Click the '-' button once for 'iPhone X'.
 * 10. Verify that the total price is changed correctly.
 */
public class TC_020_SP_003 extends Hooks {
    ...
    void verifyShoppingCartProcessFunctionsProperly() {
        ...
        boolean areTheseProductsAddedToShoppingCart = pages.getSmartphonesPage().areTheseProductsAddedToShoppingCart("OPPOF19", "Samsung Universe 9", "iPhone X");
        Assert.assertTrue(areTheseProductsAddedToShoppingCart, "There should be three products in the shopping cart but there is not");
        ...
        Assert.assertTrue(isTotalPriceInShoppingCartCorrect, "Total price in shopping cart does not match with the total price of product(s)");
    }
}